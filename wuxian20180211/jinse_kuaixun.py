    ...
    def getDatas(self, index):
        ...
        for item in all_div:
            if item['day_name'] == '今天':
                ...
                print(insertStr)
                self.dao.saveInfo(tableName="tbl_kuaixun", columesName="id,createtime,content,sourceurl,source", values=insertStr)
            else:
                return None
        return item['id']

    def update(self):
        self.ids = self.dao.getIdsBySource("tbl_kuaixun", utils.gettoday(),"金色财经")
